...
def render():
    """
    🐍 蛇噬/反冲 手套炼金 页面
    在 main.py 中：import page_serpentine; page_serpentine.render()
    """
    # 1. 初始化本页面自己的状态
    if "snake_gloves" not in st.session_state or "snake_weapons" not in st.session_state:
        g, w = load_data()
        st.session_state.snake_gloves = g
        st.session_state.snake_weapons = w

    gloves = st.session_state.snake_gloves
    weapons = st.session_state.snake_weapons
    ...
    sel_glove = st.sidebar.selectbox(
        "选择手套：",
        glove_names,
        key="snake_sel_glove"
    )
    ...
    btn_g1 = col1.button("🧤 刷新当前", key="snake_btn_g1")
    btn_g2 = col2.button("🔁 刷新全部", key="snake_btn_g2")
    ...
    sel_weapon = st.sidebar.selectbox(
        "选择枪：",
        weapon_names,
        key="snake_sel_weapon"
    )
    cur_weapon = next(w for w in weapons if w["name"] == sel_weapon)

    col3, col4 = st.sidebar.columns(2)
    btn_w1 = col3.button("🔫 刷新当前枪", key="snake_btn_w1")
    btn_w2 = col4.button("💥 刷新全部枪", key="snake_btn_w2")
    ...
    if st.button("计算最大可用材料磨损", key="snake_btn_calc_inverse"):
    ...
    for i in range(5):
        ...
        with c2:
            wear = st.number_input(
                ...
                step=0.0001,
                format="%.6f",
                key=f"snake_mat_wear_{i}"
            )
        with c3:
            st.caption(f"允许磨损区间：[{m_min:.2f} ~ {m_max:.2f}]（当前选择：{name}）")
        mat_sel.append((name, wear))

    if st.button("计算合成手套磨损", key="snake_btn_calc_forward"):
        mapped = []
        for (n, w) in mat_sel:
        ...
        if mapped:
            ...
            if tier:
                st.info(
                    f"预计成色：**{tier}**  （区间："
                    f"{GLOVE_TIER[tier][0]:.2f} ~ {GLOVE_TIER[tier][1]:.2f}）"
                )
            else:
    ...
    ax2.bar(x_idx, sorted_prices, color="#ff9966")
